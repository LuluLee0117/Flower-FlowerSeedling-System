...
if (isset($_POST["send"])) {  // 是否是表單送回
    $sql = "SELECT * FROM `靜止戶資料表` WHERE id='$id'";
    ...
    $sql = "INSERT INTO `客戶資料表` (`name`, `id`, `birth`, `tel`, `email`, `age`, `photo`, `discount`) VALUES ('$name','$id','$birth','$tel','$email','$age','$photo','$discount')";
    if (!mysqli_query($db, $sql)) // 執行SQL指令
        ...
        $photo = $row["photo"];
        $discount = $row["discount"];
        $sql = "DELETE FROM `靜止戶資料表` WHERE id='$id'";
        if (!mysqli_query($db, $sql)) // 執行SQL指令
            $result = "刪除記錄失敗...<br/>" . mysqli_error($db);
        else
    ...
    <center>
        ...
        <body background="weather.png">
            ...
            <form method="post" action="">
                <input type="submit" name="send" value="刪除記錄" style="background-color:#FFD382;border-color:#FFD382" />
            </form>
            <?php echo $result ?>
        </body>
